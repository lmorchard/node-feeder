...
_.extend(CouchSync.prototype, HashSync.prototype, {
    ...
    sync_read_collection: function (coll, options) {
        ...
        var _super = function () {
            HashSync.prototype.sync_read_collection.apply(this, arguments);
        };
        
        var design_match = /\/(.*)\//.exec(_.result(coll, 'url'));
        if (!design_match) { return _super(); } 

        this.db.view(design_match[1], 'all', function (err, body) {
            if (err) { return _super(); }
            options.success($this._sortLimitItems(coll, options, body.rows));
        });
    },
    _set: function (key, val, next) {
        var $this = this;
