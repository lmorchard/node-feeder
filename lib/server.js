...
_.extend(Server.prototype, {
    ...
    init: function (options) {
        ...
        this.feed_items = new models.FeedItemCollection();

        this.resources.on('all', function (ev, r) {
            // util.debug('EV ' + ev + ' ' + r);
        });

        this.resources.on('poll:end', function (resource) {
            $this.feed_items.parseResource(resource, 
    ...
    viewFeeds: function (req, res) {
        ...
        resources.fetch({
            success: function (r_coll, resp, options) {
                
                var r_map = {};
                r_coll.each(function (r) {
                    r_map[r.get('resource_url')] = r.toJSON();
                });
                    ...
                    success: function (coll, resp, options) {
                        res.render('feeds', {
                            ...
                            items: coll.map(function (item) {
                                var out = item.toJSON();
                                out.resource = r_map[out.resource_url];
                                var parsed = url.parse(out.resource_url);
