...
Resource = BaseModel.extend({
    ...
    poll: function (options, next) {
        ...
        var _save = function (err) {
            $this.save({last_validated: t_now}, {
                success: function (model, resp, options) {
                    ...
                    var status_ev = 'poll:status_' + status_code;
        ...
        var req = request(opts, function (err, resp, body) {
            ...
            if (200 != resp.statusCode) { return _save(); }

            if (zlib && resp.headers['content-encoding'] && 
                    resp.headers['content-encoding'].toLowerCase().indexOf('gzip') !== -1) {
                var buf = new Buffer(body);
                zlib.gunzip(buf, function (err, unbuf) {
                    var body = err ? '' : unbuf.toString($this.get('encoding'));
                    $this.set('body', body);
                    return _save();
                });
            } else {
                if (body) {
                    $this.set('body', body.toString($this.get('encoding')));
                }
                return _save();
            }
        });
